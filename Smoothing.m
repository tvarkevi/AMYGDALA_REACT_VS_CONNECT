function log_fname = Smoothing(iSubject, subject_ID, ~, t2_dir, scans_dir, prefix, study_ID, working_dir, log_fname)

% Preprocessing of the fMRI data: Smoothing
%
% Input arguments:
...
%
% List of open inputs:
%       inputs{1} = Smooth: Images to smooth - cfg_files


inputs = {};

% ----- Print progress to command window ----- %
fprintf(['\n' num2str(iSubject) '\tSmoothing for subject: \t' subject_ID '\n']);
...
% ----- Run preprocessing ----- %
jobfile = {[working_dir '\Smoothing_job.m']};
try
