function VoxelMatchedRegressionAnalysis(workdir, subjects_fn, rs_dir, emo_dir, connectivty_prefix, reactivity_fn, mask_fn, fwhm, n_perms, n_tests)

% Conduct voxel-matched linear regression analysis.
%
% Input arguments:
%       workdir = 'E:\AMYGDALA_RECON\Analysis';
%       subjects_fn = 'voxel_matched_regression_analysis.txt';
%       rs_dir = 'E:\AMYGDALA_RECON\Data\NIFTI_GRAND_REST';
%       emo_dir = 'E:\AMYGDALA_RECON\Data\NIFTI_GRAND_EMO';
...
R.nIts = n_perms; % number of permutations
R.nTests = n_tests; % FWER-correction factor

% ----- Read text files with subject ID's ----- %
subjects_list = importdata([workdir '\' subjects_fn]);
...
for iSubject = 1:length(subjects_list)
    ...
    % ----- Extract task-based parametric map for this subject ----- %
    this_subject_react_map_ID = dir([emo_dir '\' this_subject '\' this_subject '*\first_level_analysis\' reactivity_fn]);
    
    H = spm_vol([this_subject_react_map_ID.folder '\' this_subject_react_map_ID.name]);
    D = spm_read_vols(H);
    
    spm_smooth(D, D, S.smoothing_fwhm);
    
    S.reactivity_maps{iSubject} = D;
end
...
[R.nBlobsReal, R.maxBlobPowerReal, R.info, R.sumTReal, R.L, R.sumT, R.Tcorr, R.tcrit, R.Traw] = inner_voxel_matched_regression_clustering(S, p);
...
% output_dir = [workdir '\software_validation_voxel_matched_regression_' connectivty_prefix 'vs_' reactivity_fn(1:end-4)];
output_dir = [workdir '\voxel_matched_regression_' connectivty_prefix 'vs_' reactivity_fn(1:end-4)];
...
for iIt = 1:R.nIts
    ...
    % ----- Conduct voxel-matched regression clustering ----- %
    [nBlobs0, maxBlobPower0, ~, ~, ~, ~, ~, ~, ~] = inner_voxel_matched_regression_clustering(S, p);
...
function [nBlobs, maxBlobPower0, info, sumtv, L, sumT, T, tcrit, Traw] = inner_voxel_matched_regression_clustering(S, p)
% Conduct voxel-matched regression and apply landscape-based cluster
...
for iVoxel = 1:length(vox_ind)
    ...
    for iSubject = 1:length(S.subject_names)
        ...
        y(iSubject) = S.reactivity_maps{p(iSubject)}(this_voxel_index);
    end
